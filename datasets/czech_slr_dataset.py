...
from datasets.normalization.body_normalization import BODY_IDENTIFIERS
from datasets.normalization.hand_normalization import HAND_IDENTIFIERS
from datasets.normalization.body_normalization import normalize_single_dict as normalize_single_body_dict
from datasets.normalization.hand_normalization import normalize_single_dict as normalize_single_hand_dict
...
def dictionary_to_tensor(landmarks_dict: dict, max_len: int) -> torch.Tensor:
    ...
    if output.shape[0] > max_len:
        output = output[np.linspace(0, output.shape[0], max_len, endpoint = False, dtype=np.int32)]
...
class CzechSLRDataset(torch_data.Dataset):
    ...
    def __init__(self, dataset_filename: str, num_labels=5, transform=None, augmentations=False,
                 augmentations_prob=0.5, max_frames = 160, normalize=True):
        """
    ...
    def __getitem__(self, idx):
        ...
        depth_map = dictionary_to_tensor(depth_map, self.max_frames)
        ...
        if depth_map.shape[0] < self.max_frames:
            temp = torch.zeros((self.max_frames, depth_map.shape[1], depth_map.shape[2]))
            temp[:depth_map.shape[0], :, :] = depth_map
            depth_map = temp

            # depth_map = F.interpolate(depth_map.unsqueeze(0), size=(self.max_frames, depth_map.shape[1], depth_map.shape[2]), mode='linear', align_corners=False)
